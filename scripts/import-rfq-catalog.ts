...
async function main() {
  ...
  await jobCol.bulkWrite(allJobs.map((j) => ({ updateOne: { filter: { jobCode: j.jobCode }, update: { $set: { ...j, version, active: true }, $setOnInsert: { firstVersion: version } }, upsert: true } })))
  await partCol.bulkWrite(allParts.map((p) => ({ updateOne: { filter: { sheet: p.sheet, sku: p.sku }, update: { $set: { ...p, version, active: true }, $setOnInsert: { firstVersion: version } }, upsert: true } })))
