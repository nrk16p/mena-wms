...
**เพิ่มจากการค้นคว้าแนวปฏิบัติจัดซื้อ (2026-09-08):** ฐาน VAT ต่อ supplier, วันที่/วันหมดอายุใบเสนอราคา, บังคับเหตุผลเมื่อไม่เลือกรายถูกสุด, ขั้นต่ำ 3 ใบเสนอราคาหรือระบุเหตุผล (ที่มา: ControlHub, AuraVMS, SpecLens bid-tabulation guide)

**ไม่ทำในรอบนี้ (YAGNI):** การให้คะแนนถ่วงน้ำหนัก (weighted scoring), ลงมติในระบบโดยกรรมการเอง, inbox รออนุมัติ, เชื่อม PR/PO ของ ATMS อัตโนมัติ, public API, ราคากลางเทียบอัตโนมัติ
...
type PriceCompare = {
  ...
  selectionReason: string            // เหตุผลที่เลือก — บังคับกรอกเมื่อรายที่เลือกไม่ใช่สุทธิต่ำสุด
  fewerQuotesReason: string          // เหตุผลที่มีใบเสนอราคาครบน้อยกว่า 3 ราย (เช่น มีผู้ขายรายเดียว)
...
type PriceCompareSupplier = {
  ...
  vatMode: "excl" | "incl" | "none"   // ฐานราคาที่เสนอ: ก่อน VAT (default) / รวม VAT แล้ว / ไม่มี VAT (ผู้ขายไม่จดทะเบียน)
  quoteDate: string        // YYYY-MM-DD วันที่ในใบเสนอราคา
  validUntil: string       // YYYY-MM-DD ใบเสนอราคาใช้ได้ถึง — เตือนเมื่อเลยวันนี้
  conditions: {
...
| `supplierTotals(doc, idx)` | `{ subtotal, discount, afterDiscount, vat, net }` ตาม `vatMode`: **excl** → vat = afterDiscount × 7%, net = afterDiscount + vat · **incl** → net = afterDiscount, vat = net − net/1.07 (แยกให้เห็น) · **none** → vat = 0, net = afterDiscount — ทำให้ทุก supplier เทียบกันที่ "สุทธิที่ต้องจ่ายจริง" |
| `isQuoteExpired(supplier, today)` | true เมื่อ validUntil < today |
| `lowestPerLine(doc)` | index ของ supplier ที่ถูกสุดต่อแถว (เฉพาะรายที่เสนอ) |
| `lowestNet(doc)` | index ของ supplier ที่สุทธิต่ำสุด |
| `isComplete(doc)` | ครบเงื่อนไขปิดใบ: มีรายการ ≥1, supplier **≥3** ที่มีราคาครบทุกแถว (หรือ ≥1 + `fewerQuotesReason`), กรรมการมีชื่อครบ 4, selectedSupplier ไม่ว่าง, ถ้ารายที่เลือก ≠ สุทธิต่ำสุดต้องมี `selectionReason` |
...
รูปแบบ `PC-YYMM-NNN`: YY = ปี ค.ศ. 2 หลักท้าย (ตามฟอร์มต้นแบบ: 7 ก.ย. 2569 = ค.ศ. 2026 → `PC-2609-002`), MM = เดือน, NNN = ลำดับในเดือนนั้น เริ่ม 001
...
3. **เงื่อนไขในการคัดเลือก** — ตาราง 6 ข้อ × supplier (input ข้อความสั้น) ตามฟอร์ม + แถว "วันที่ใบเสนอราคา / ใช้ได้ถึง" (date) และ "ฐานราคา" (ก่อน VAT / รวม VAT / ไม่มี VAT) ต่อ supplier
...
6. **สรุปผล** — ผู้ได้รับเลือก (radio Supplier 1–N พร้อมชื่อ+สุทธิ), ช่อง "เหตุผลที่เลือก" **บังคับ** เมื่อรายที่เลือกไม่ใช่สุทธิต่ำสุด (บล็อกส่งลงนาม), ช่อง "เหตุผลที่มีใบเสนอราคาน้อยกว่า 3 ราย" แสดงและบังคับเมื่อ supplier ที่ราคาครบ < 3, ป้ายเตือน "ใบเสนอราคาหมดอายุ" ต่อ supplier ที่ validUntil เลยวันนี้
7. **ประวัติ** — timeline จาก `price_compare_log` (collapsed)

พฤติกรรมสถานะ:
- ร่าง → รอลงนาม: ต้องผ่าน `isComplete()` ยกเว้นชื่อกรรมการ (กรอกทีหลังได้) — รวมกฎ 3 ราย/เหตุผล และเหตุผลเมื่อไม่เลือกรายถูกสุด
...
| สรุปยอด | รวมก่อนภาษี / ส่วนลด / รวมหลังส่วนลด / VAT 7% / รวมทั้งหมด (สุทธิ) ต่อ supplier — ช่อง VAT พิมพ์ "ไม่มี VAT" เมื่อ none และ "(รวมในราคา)" เมื่อ incl |
| เงื่อนไข | 6 ข้อ × supplier (ข้อ 6 แยก ขA / ขB) + แถว "(7) วันที่ใบเสนอราคา / ใช้ได้ถึง" |
| เหตุผล | ใต้ตารางกรรมการ: "เหตุผลที่เลือก: …" และ "เหตุผลที่มีใบเสนอราคาน้อยกว่า 3 ราย: …" (พิมพ์เฉพาะที่มีข้อความ) |
