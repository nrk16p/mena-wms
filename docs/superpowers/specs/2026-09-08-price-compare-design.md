...
1. **หัวเอกสาร** — ชื่อสินค้า/งานซ่อม, หน่วยงานที่ร้องขอ (combobox จาก DEPT_MASTER ของ order-tracking + พิมพ์เอง), ผู้จัดทำ (อ่านอย่างเดียว), ครั้งที่แก้ไข/วันที่ (อ่านอย่างเดียว), ลิงก์อ้างอิง (PR / ทะเบียน / เบอร์รถ — ทะเบียนใช้ autocomplete `vehicle_master` แบบ repair-external)
