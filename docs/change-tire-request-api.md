...
> API แจ้งเตือน "ยางถึงกำหนดเปลี่ยน" (เตือนล่วงหน้าก่อนยางหมด) ดูที่ `docs/tire-due-api.md`
...
└──────────────────┬───────────────────────────────────┘
   ...
   ┌── เรียก endpoint เดียว (แนะนำ ดูข้อ 3) ───────────┐
   │ GET /api/tire-change-request/lookup               │
   │   → ประวัติยาง + ข้อมูลรถ + สต๊อก + สถานะคำขอ      │
   │   + ค่าคำนวณครบทุกคอลัมน์ (server คำนวณให้)         │
   └──────────────────┬───────────────────────────────┘
                      ▼ แสดงตารางยางรายเส้น
...
## 3. ขั้นค้นหา — แนะนำ: ใช้ lookup endpoint เดียวจบ ⭐
...
{
  "vehicle": {
    "plate": "สบ.71-3569", "vehicleType": "Mixer 10 ล้อ", "brand": "HINO",
    "model": "FM1AKKM", "fleet": "Asia", "fleetNo": "ME053", "plant": "สรงประภา"
  },
  "odometer": 290000,
  "total": 10,
  "items": [
    {
      "_id": "...",
      "vehicle": "สบ.71-3569",
      "tirePosition": "RA1ล้อหลังคู่หน้าซ้ายเส้นนอก",
      "positionCode": "RA1",
      "positionName": "ล้อหลังคู่หน้าซ้ายเส้นนอก",
      "product": "ยางผ้าใบดอกบั้ง 1000-20",
      "serialNo": "TCS-DS C5Y31665",
      "treadMm": 16,
      "mileageStart": 248234,
      "changeIn": "2025-06-04T03:00:00.000Z",
      "isLatest": true,
      ...
      "remainingLevel": "red",
      "bahtPerKm": 0.1069,
      "age": { "text": "1 ปี", "level": "warn" },
      "request": { "itemStatus": "approved", "requestStatus": "approved", "appointmentDate": null }
    }
  ]
}
```
...
| Field | คอลัมน์ | หมายเหตุ |
|---|---|---|
| `vehicle` (object) | card ข้อมูลรถ + เติมช่อง ฟลีท/Plant | `null` = แสดง "ไม่พบในทะเบียนยานพาหนะ" |
| `unitPrice` / `stockDistance` | Unit Price / ระยะทาง | `null` = "—" |
| `usedDistance` | ระยะทางใช้งาน | `null` = "—" |
| `remainingPct` + `remainingLevel` | ประสิทธิภาพคงเหลือ | badge สี: `green` / `amber` / `red` — ติดลบได้ |
| `bahtPerKm` | บาทต่อกิโล | แสดงทศนิยม 2–4 ตำแหน่ง |
| `age.text` + `age.level` | ระยะเวลาใช้งาน | `normal` = ข้อความเฉย ๆ, `warn` = badge เหลือง, `danger` = badge แดง |
| `request` | คอลัมน์ขอเปลี่ยน | `null` = ปุ่มขอเปลี่ยนยาง; ไม่ null ดู logic ข้อ 4 |
| `isLatest` | highlight แถวเขียว | ยางที่อยู่บนรถตอนนี้ (เรียงขึ้นก่อนแล้ว) |

> หน้าเว็บ production ใช้ endpoint นี้ตัวเดียวกัน — ผลลัพธ์จะตรงกันเสมอ

---

## 3b. API ดิบรายตัว (ถ้าต้องการแยกเรียกเอง)
...
## 4. การแสดงผลตารางยางรายเส้น (สูตรอ้างอิง — lookup endpoint คำนวณให้แล้ว)
